<div class="flex flex-col relative" x-data="{ open: true }" x-on:click.away="open = false; @this.resetIndex();" x-on:keydown.escape="open = false; @this.resetIndex();">
    <input type="text" class="focus:outline-none focus:border-purple-800 border-2 w-56 rounded md:mr-5 py-1 px-2" @click.prevent="open = true" wire:model="query"
    ...
    @if (strlen($query) >= 2)
        ...
        @if (count($jobs) > 0)
            @foreach ($jobs as $index => $job)
                <a href="#" class="{{ ($index === $selectedIndex) ? 'text-purple-800' : '' }} my-2">{{ $job['title'] }}</a>
            @endforeach
        @else
